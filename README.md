...
├── README.md                       # 本ファイル
├── LICENSE                         # MIT
├── requirements.txt                # 依存関係 (loose)
├── requirements.txt.lock           # uv pip compile による pinned 版
├── .env.example                    # API キーと環境変数のテンプレ
├── .gitignore
...
│   ├── replay_recorder.py          # graph.stream を JSON stage に変換 (replay capture)
│   └── search_tool.py              # Tavily ツールラッパー
├── examples/
│   ├── weather_ai_demo.md          # 実行結果サンプル (Round 2 fact-correction 観察可)
│   └── replays/                    # 録画済み実行ログ (JSON、--save-replay-as で生成)
├── docs/
│   ├── DESIGN.md                   # 設計 5 原則の詳細
│   └── KNOWLEDGE.md                # 実装・運用上の知見
├── scripts/                        # 補助スクリプト
├── assets/                         # アーキテクチャ図 + LangSmith trace 画像
└── tests/
